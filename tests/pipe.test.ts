...
const toStringValue = (value: number) => value.toString();
const replaceFalsyValue = (value: string) => (value.length ? value : '-');
describe('pipe테스트', () => {
  it('4에서 연속된 함수(add10, add5)를 통한 결과 19를 받을 수 있다', () => {
    const sum = pipe(add10, add5, toStringValue, replaceFalsyValue);
    const result = sum(4);
